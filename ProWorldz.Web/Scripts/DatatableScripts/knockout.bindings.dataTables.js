...
(function () {
    ...
    function convertDataCriteria (srcOptions) {
        ...
        for (var i = 0; i < srcOptions.length; i++) {
            if (srcOptions[i].name == "columns") {
                for (var j = 0; j < srcOptions[i].value.length; j++)
        ...
        ko.utils.arrayForEach(srcOptions, function (item) {
            ...
            else if (cog.utils.string.startsWith(item.name, "search1"))
                destOptions.Columns[colIndex].SearchText = item.value;
            else if (cog.utils.string.startsWith(item.name, "columns"))
            {
                for (var index = 0; index < item.value.length; index++)
                {
                    //destOptions.Columns[index].push(item.value[index]);
                    // destOptions.Columns[index].SortOrder = item.value[index].data;

                    destOptions.Columns[index].ColumnName = item.value[index].data;
                    destOptions.Columns[index].IsSorted = item.value[index].orderable;
                    destOptions.Columns[index].IsSearchable = item.value[index].searchable;
                    destOptions.Columns[index].SortDirection = "";
                }
            }
            else if (cog.utils.string.startsWith(item.name, "order")) {
                
                for (var index = 0; index < item.value.length; index++)
                {
                    destOptions.Columns[item.value[index].column].SortDirection = item.value[index].dir;
                }
